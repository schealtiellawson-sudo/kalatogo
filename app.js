...
async function loadDashOverview() {
  ...
  function _applyScoreGauge(sNote, sAvis, _note = 0, _nbAvis = 0) {
    ...
    const scoreSubEl     = document.getElementById('kpi-score-sub');
    const scoreProgEl    = document.getElementById('kpi-score-prog-label');
    const scoreTrendEl   = document.getElementById('kpi-score-trend');
    ...
    if (scoreSubEl)   scoreSubEl.textContent   = _nbAvis > 0 ? `${_note.toFixed(1)}/5 · ${_nbAvis} avis` : 'Pas encore d\'avis';
    if (scoreProgEl)  scoreProgEl.textContent  = `${s} / 100`;
    if (scoreTrendEl) scoreTrendEl.textContent = nL ? `↗ +${nL.min - s} pts vers ${nL.emoji} ${nL.name}` : '🏆 Score maximum !';
  ...
  // ── Completion — Direction C : hint + CTA unique ──
  const firstMissing = completionItems.find(i => !i.done);
  const missingCount = completionItems.filter(i => !i.done).length;
  const hintEl = document.getElementById('kpi-completion-hint');
  ...
  if (ctaEl) {
    if (firstMissing) {
      const actionLabels = { photos: 'Ajouter mes photos maintenant →', profil: 'Compléter mon profil →', dispo: 'Activer ma disponibilité →' };
      ctaEl.textContent = actionLabels[firstMissing.action] || 'Compléter mon profil →';
      ctaEl.onclick = () => showDashSection(firstMissing.action);
    } else {
      ctaEl.textContent = 'Voir mon profil public →';
      ctaEl.onclick = () => viewMyPublicProfil();
    }
  }
...
function _getFondateurMessages() {
  ...
  return arr.filter(m => m && (m.type === 'message_fondateur' || m.type === 'demande_client')
    && (!m.created_at || new Date(m.created_at) <= now));
}
  ...
  try {
    ...
    if (createOk) {
      ...
      try {
        ...
        const bodyJ0 = `Coucou ${prenom},\n\nC'est Schealtiel - fondateur WOZALI.\n\nJe veux juste te dire quelque chose avant tout : tu as fait ce que beaucoup ne font pas. Tu as décidé que ça allait changer. La plupart des gens restent là où ils sont en espérant que ça change tout seul.\n\nToi tu as agi. Et ça mérite d'être célébré. Vraiment.\nBienvenue dans une nouvelle étape.\n\n---\n\nMaintenant je vais être direct avec toi.\n\nJ'ai créé WOZALI parce que j'ai vu des gens sérieux ne pas avoir de quoi manger, pendant que d'autres - moins compétents, mais mieux nés, de bonne famille - n'avaient même pas besoin de travailler pour vivre bien.\n\nParce que des femmes qualifiées se faisaient demander leur corps juste pour décrocher un emploi.\n\nParce que dans ce pays, si tu n'as pas le bon contact, ton talent ne vaut rien aux yeux du marché.\n\nWOZALI est née de cette douleur-là. Pas d'une idée sur un tableau blanc. De ce que j'ai vu, entendu, ressenti au plus profond de moi en regardant des gens travailler dur sans que ça leur revienne.\n\nWOZALI existe pour que ton travail, ta valeur, soient ce qui parlent pour toi.\n\nPas ton réseau. Pas ta famille. Pas les personnes que tu connais. Et pas parce que vendre ton corps pour pouvoir manger est ta seule option.\n\nMaintenant tu as un profil. Ta valeur existe.\nEt si tu utilises vraiment cette plateforme faite pour toi, ton activité va prendre un tournant que tu n'imagines pas encore.\n\n---\n\nAujourd'hui, fais une chose : partage ton lien WOZALI à 5 personnes - des clients, des gens qui te connaissent, qui ont vu ton travail. Demande-leur de mettre un avis honnête sur ton profil.\n\nPourquoi ? Parce que chaque avis, c'est un nouveau client qui hésite entre toi et quelqu'un d'autre - et qui te choisit. C'est de l'argent qui rentre demain grâce à ce que tu fais aujourd'hui.\n\nDans les prochains jours, je t'envoie des messages pour t'aider à prendre tes marques. Reste connecté.\n\nEncore une fois, bienvenu.\n\nSchealtiel`;
      ...
      } catch(eWelcome) { console.warn('[welcome msgs]', eWelcome); }
...
function switchAgentTab(tab) {
  ['candidatures', 'actifs', 'kpi', 'docs'].forEach(t => {
    ...
    if (!btn || !panel) return;
    const active = t === tab;
    btn.style.color = active ? '#E8940A' : 'rgba(252,224,168,.4)';
    btn.style.borderBottomColor = active ? '#E8940A' : 'transparent';
  ...
  container.innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-bottom:20px;">
      ...
      <div style="background:rgba(249,115,22,.08);border:1px solid rgba(249,115,22,.2);border-radius:12px;padding:14px;text-align:center;">
        ...
        <div style="font-size:10px;color:var(--gris);text-transform:uppercase;letter-spacing:1px;margin-top:2px;">Cartons orange</div>
      </div>
      <div style="background:rgba(220,38,38,.08);border:1px solid rgba(220,38,38,.2);border-radius:12px;padding:14px;text-align:center;">
        <div style="font-family:'Geist Mono',monospace;font-size:26px;font-weight:900;color:#dc2626;">${rouges}</div>
      ...
      <div style="background:#1E180E;border:1px solid rgba(232,148,10,.3);border-radius:16px;padding:24px;max-width:560px;width:100%;max-height:80vh;overflow-y:auto;">
        ...
        <table style="width:100%;border-collapse:collapse;">
          <thead><tr style="background:rgba(232,148,10,.08);">
            ...
            <th style="padding:8px;text-align:center;font-size:11px;color:var(--gris);text-transform:uppercase;letter-spacing:1px;">Inscrits</th>
            <th style="padding:8px;text-align:center;font-size:11px;color:var(--gris);text-transform:uppercase;letter-spacing:1px;">Pro</th>
  ...
  tabs.forEach(t => {
    ...
    if (btn) {
      if (t === tab) {
        ...
        btn.style.borderColor = '#E8940A';
      } else {
        btn.style.background = 'rgba(255,255,255,0.06)';
        btn.style.color = 'rgba(252,224,168,0.65)';
        btn.style.borderColor = 'rgba(232,148,10,0.25)';
      }
    }
  });
}
...
async function publierOffre() {
  ...
  const body = {
    fields: {
      'Titre': titre,
      'Métier': metier,
      'Pays': ({'Togo':'TG','Bénin':'BJ'}[get('recrut-pays')]) || undefined,
