// ══════════════════════════════════════════
// WOZALI Business Suite — Module 4 : Annonces équipe (patron)
...
(function(){
  ...
  async function loadAnnoncesPatron(){
    ...
    if (!window.currentPrestataire?.id) {
      root.innerHTML = `<div style="padding:40px;text-align:center;color:rgba(252, 224, 168,.5);">Connecte-toi pour diffuser des annonces.</div>`;
      return;
    }
      ...
      <div style="font-family:'Geist',sans-serif;color:#FCE0A8;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;flex-wrap:wrap;gap:12px;">
          <div>
            ...
            <p style="font-size:13px;color:rgba(252, 224, 168,.5);margin:0;">Diffuse un message à toute ton équipe. WhatsApp + dashboard employé.</p>
          </div>
          <button onclick="openNouvelleAnnonceModal()" style="background:#E8940A;color:#14100A;border:none;padding:10px 20px;border-radius:10px;font-weight:700;font-size:13px;cursor:pointer;">+ Nouvelle annonce</button>
        </div>
  ...
  function emptyHTML(){
    return `<div style="background:rgba(232,148,10,.03);border:1px dashed rgba(232,148,10,.2);border-radius:14px;padding:30px;text-align:center;">
      ...
      <p style="color:rgba(252, 224, 168,.5);margin:0;">Aucune annonce diffusée.</p>
    </div>`;
  }
    ...
    return `<div style="display:grid;gap:10px;">
      ${state.annonces.map(a => {
        ...
        return `<div style="background:rgba(232,148,10,.06);border:1px solid rgba(232,148,10,.15);border-radius:12px;padding:14px;">
          ...
          <div style="display:flex;gap:6px;margin-top:10px;font-size:10px;color:rgba(252, 224, 168,.45);">
  ...
  function skeletonHTML(){
    return `<div style="padding:40px;text-align:center;color:rgba(252, 224, 168,.4);">Chargement annonces…</div>`;
  }
  ...
  window.openNouvelleAnnonceModal = function(){
    ...
    modal.innerHTML = `
      <div id="annonce-modal-bg" style="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:10000;display:flex;align-items:center;justify-content:center;padding:20px;" onclick="if(event.target===this)this.remove()">
        <div style="background:#14100A;border:1px solid rgba(232,148,10,.3);border-radius:16px;padding:24px;max-width:500px;width:100%;font-family:'Geist',sans-serif;color:#FCE0A8;">
          <h2 style="font-family:'Fraunces',serif;font-size:22px;margin:0 0 16px;">Nouvelle annonce</h2>
          <input id="ann-titre" type="text" placeholder="Titre (ex: Réunion demain 9h)" style="width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(232,148,10,.2);border-radius:10px;padding:10px 12px;color:#FCE0A8;margin-bottom:10px;font-family:inherit;">
          <textarea id="ann-msg" placeholder="Message…" rows="5" style="width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(232,148,10,.2);border-radius:10px;padding:10px 12px;color:#FCE0A8;margin-bottom:14px;font-family:inherit;resize:vertical;"></textarea>
          <label style="display:flex;align-items:center;gap:8px;font-size:12px;margin-bottom:16px;color:rgba(252, 224, 168,.7);">
            <input id="ann-wa" type="checkbox" checked> Diffuser aussi par WhatsApp
          </label>
          <div style="display:flex;gap:10px;">
            <button onclick="document.getElementById('annonce-modal-bg').remove()" style="flex:1;background:rgba(255,255,255,.06);border:1px solid rgba(252, 224, 168,.15);color:#FCE0A8;padding:10px;border-radius:10px;font-weight:600;cursor:pointer;">Annuler</button>
            <button onclick="submitAnnonce()" style="flex:2;background:#E8940A;border:none;color:#14100A;padding:10px;border-radius:10px;font-weight:700;cursor:pointer;">Diffuser à l'équipe</button>
          </div>
          <div id="ann-status" style="font-size:11px;color:rgba(252, 224, 168,.55);margin-top:10px;text-align:center;"></div>
        </div>
      </div>`;
  ...
  window.submitAnnonce = async function(){
    ...
    try {
      ...
      const res = await wFetch('/api/wozali-pay/annonces-broadcast', {
        method: 'POST',
