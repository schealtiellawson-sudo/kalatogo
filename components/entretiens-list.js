...
(function () {
  ...
  async function load(containerId) {
    ...
    if (!window.currentUser?.id) {
      container.innerHTML = `<div style="text-align:center;padding:60px;color:rgba(252, 224, 168,.4);">Connecte-toi pour voir tes entretiens.</div>`;
      return;
    }
    ...
    try {
      ...
      if (all.length === 0) {
        container.innerHTML = `<div style="text-align:center;padding:60px 20px;color:rgba(252, 224, 168,.5);font-family:Poppins,sans-serif;">
          <div style="font-size:48px;margin-bottom:12px;">📅</div>
          <p style="font-size:15px;margin:0 0 6px;">Aucun entretien planifié.</p>
          <p style="font-size:12px;margin:0;color:rgba(252, 224, 168,.35);">Quand un recruteur planifie un entretien avec toi, il apparaît ici.</p>
        </div>`;
        return;
      }
      ...
      container.innerHTML = `
        ${upcoming.length ? `
          <div style="margin-bottom:24px;">
            <div style="font-family:'Geist Mono',monospace;font-size:11px;color:#E8940A;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:10px;">À venir (${upcoming.length})</div>
        ...
        ${past.length ? `
          <div>
            <div style="font-family:'Geist Mono',monospace;font-size:11px;color:rgba(252, 224, 168,.45);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:10px;">Historique (${past.length})</div>
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:12px;">
  ...
  function renderCard(e) {
    ...
    return `
      <div style="background:rgba(255,255,255,.04);border:1px solid rgba(232,148,10,.12);border-radius:14px;padding:16px;font-family:Poppins,sans-serif;color:#FCE0A8;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;gap:8px;">
          <div>
            <div style="font-size:11px;color:rgba(252, 224, 168,.45);text-transform:uppercase;letter-spacing:1px;">${TYPE_LABEL[e.type] || e.type}</div>
            <div style="font-family:Fraunces,serif;font-size:16px;font-weight:700;color:#FCE0A8;margin-top:2px;">${escapeHtml(e.offre_titre || 'Entretien')}</div>
          </div>
        ...
        <div style="font-size:12px;color:rgba(252, 224, 168,.55);margin-bottom:8px;">📅 ${fmtDate(e.date_heure)}</div>
        ${lieu ? `<div style="font-size:12px;color:rgba(252, 224, 168,.55);margin-bottom:8px;word-break:break-all;">📍 ${escapeHtml(lieu)}</div>` : ''}
