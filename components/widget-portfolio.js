...
(function () {
  ...
  function projetTile(p) {
    ...
    return `
      <article style="background:#16201a;border:1px solid rgba(232,148,10,.18);border-radius:14px;overflow:hidden;display:flex;flex-direction:column;">
        ${cover ? `<div style="aspect-ratio:4/3;background:#000 url('${esc(cover)}') center/cover no-repeat;"></div>` : `<div style="aspect-ratio:4/3;background:#14100A;display:flex;align-items:center;justify-content:center;color:rgba(232,148,10,.4);font-size:24px;">📷</div>`}
        <div style="padding:12px 14px;">
          ${p.categorie ? `<div style="font-family:'Geist Mono',monospace;font-size:10px;letter-spacing:.06em;color:rgba(232,148,10,.85);text-transform:uppercase;margin-bottom:4px;">${esc(p.categorie)}</div>` : ''}
          <div style="font-weight:700;font-size:15px;color:#FCE0A8;">${esc(p.titre)}</div>
          ${p.description ? `<div style="font-size:12px;color:rgba(252, 224, 168,.65);margin-top:6px;line-height:1.4;">${esc(p.description)}</div>` : ''}
          ${p.prix_indicatif_fcfa ? `<div style="font-family:'Geist Mono',monospace;font-size:12px;color:#E8940A;margin-top:8px;">À partir de ${fmt(p.prix_indicatif_fcfa)}</div>` : ''}
        </div>
      </article>`;
  }
  ...
  R.register('portfolio', {
    async renderProfile({ proUserId, root }) {
      ...
      if (!projets.length) {
        root.innerHTML = `<div style="color:rgba(252, 224, 168,.55);font-size:13px;">Aucun projet publié pour le moment.</div>`;
        return;
      }
    ...
    async renderDashboard({ proUserId, root }) {
      ...
      root.innerHTML = `
        <div style="margin-bottom:12px;display:flex;justify-content:space-between;align-items:center;">
          <div style="font-size:13px;color:rgba(252, 224, 168,.65);">Présente tes meilleurs projets, organisés par catégorie.</div>
          <button id="wpf-add" style="background:#E8940A;color:#14100A;border:none;border-radius:10px;padding:10px 16px;font-weight:700;cursor:pointer;font-family:Poppins,sans-serif;">+ Nouveau projet</button>
        </div>
      ...
      function tile(p) {
        ...
        actions.innerHTML = `
          <button data-act="edit" style="flex:1;background:none;border:1px solid rgba(232,148,10,.4);color:#FCE0A8;border-radius:6px;padding:6px;cursor:pointer;font-size:12px;">✏️ Modifier</button>
      ...
      function refresh() {
        listEl.innerHTML = '';
        if (!projets.length) {
          listEl.innerHTML = `<div style="grid-column:1/-1;color:rgba(252, 224, 168,.55);font-size:13px;">Aucun projet pour l'instant. Ajoute ton premier — c'est ton CV visuel.</div>`;
          return;
        }
      ...
      function openForm(p) {
        ...
        ov.innerHTML = `
          <div style="background:#14100A;border:1px solid rgba(232,148,10,.3);border-radius:14px;padding:22px;max-width:540px;width:100%;color:#FCE0A8;font-family:Poppins,sans-serif;">
            <h4 style="font-family:Fraunces,serif;font-style:italic;color:#E8940A;margin:0 0 14px;">${isEdit ? 'Modifier' : 'Nouveau'} projet</h4>
            <label style="display:block;font-size:12px;color:rgba(252, 224, 168,.7);margin-bottom:4px;">Titre *</label>
            <input id="wpf-titre" value="${esc(f.titre || '')}" style="width:100%;padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;margin-bottom:10px;">
            <label style="display:block;font-size:12px;color:rgba(252, 224, 168,.7);margin-bottom:4px;">Catégorie</label>
            <input id="wpf-cat" value="${esc(f.categorie || '')}" placeholder="Logo, Mariage, Portrait…" style="width:100%;padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;margin-bottom:10px;">
            <label style="display:block;font-size:12px;color:rgba(252, 224, 168,.7);margin-bottom:4px;">Description</label>
            <textarea id="wpf-desc" rows="2" style="width:100%;padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;margin-bottom:10px;">${esc(f.description || '')}</textarea>

            <label style="display:block;font-size:12px;color:rgba(252, 224, 168,.7);margin-bottom:6px;">Photos du projet (max ${MAX_PHOTOS})</label>
            <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:10px;">
              <label id="wpf-upload-btn" style="background:#E8940A;color:#14100A;border:none;border-radius:8px;padding:8px 14px;font-weight:700;cursor:pointer;font-size:12px;display:inline-flex;align-items:center;gap:6px;">
                <span>📷 Téléverser</span>
                <input id="wpf-file" type="file" accept="image/*" multiple style="display:none;">
              </label>
              <button type="button" id="wpf-url-toggle" style="background:none;border:1px solid rgba(252, 224, 168,.2);color:#FCE0A8;border-radius:8px;padding:8px 12px;cursor:pointer;font-size:12px;">+ URL</button>
              <span id="wpf-upload-status" style="font-size:11px;color:rgba(252, 224, 168,.55);"></span>
            </div>
            ...
            <div id="wpf-url-row" style="display:none;gap:6px;margin-bottom:10px;">
              <input id="wpf-url-input" placeholder="https://i.ibb.co/..." style="flex:1;padding:8px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;font-family:'Geist Mono',monospace;font-size:12px;">
              <button type="button" id="wpf-url-add" style="background:#E8940A;color:#14100A;border:none;border-radius:8px;padding:0 14px;font-weight:700;cursor:pointer;font-size:12px;">Ajouter</button>
            </div>

            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
              <div>
                <label style="display:block;font-size:12px;color:rgba(252, 224, 168,.7);margin-bottom:4px;">Prix indicatif (FCFA)</label>
                <input id="wpf-prix" type="number" min="0" value="${esc(f.prix_indicatif_fcfa || '')}" style="width:100%;padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;">
              </div>
              <div>
                <label style="display:block;font-size:12px;color:rgba(252, 224, 168,.7);margin-bottom:4px;">Date</label>
                <input id="wpf-date" type="date" value="${esc(f.date_realisation || '')}" style="width:100%;padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;">
              </div>
            </div>
            <div style="display:flex;gap:10px;justify-content:flex-end;">
              <button id="wpf-cancel" style="background:none;border:1px solid rgba(252, 224, 168,.2);color:#FCE0A8;border-radius:8px;padding:8px 14px;cursor:pointer;">Annuler</button>
              <button id="wpf-save" style="background:#E8940A;color:#14100A;border:none;border-radius:8px;padding:8px 18px;font-weight:700;cursor:pointer;">${isEdit ? 'Enregistrer' : 'Ajouter'}</button>
            </div>
          </div>`;
