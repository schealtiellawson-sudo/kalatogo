    ...
    return `<div style="display:grid;gap:8px;">
      ${state.fiches.map(fi => {
        ...
        return `<div style="background:rgba(232,148,10,.06);border:1px solid rgba(232,148,10,.15);border-radius:10px;padding:12px;display:flex;justify-content:space-between;align-items:center;">
          ...
          <button onclick="window.open('/api/wozali-pay/paie-bulletin?id=${encodeURIComponent(fi.id)}','_blank')" style="background:rgba(255,255,255,.06);border:1px solid rgba(232,148,10,.2);color:#FCE0A8;padding:6px 10px;border-radius:8px;font-size:11px;cursor:pointer;">📄 PDF</button>
        </div>`;
      }).join('')}
    </div>`;
  }
