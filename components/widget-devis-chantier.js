...
(function () {
  ...
  function statutBadge(s) {
    const colors = {
      ...
      accepte: '#2ec27e', refuse: '#ff8080', annule: 'rgba(252, 224, 168,.4)', termine: '#2ec27e',
    };
    return `<span style="background:${colors[s] || '#666'};color:#14100A;font-family:'Geist Mono',monospace;font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;text-transform:uppercase;letter-spacing:.05em;">${esc(s.replace('_', ' '))}</span>`;
  }
  ...
  R.register('devis_chantier', {
    async renderProfile({ proUserId, fields, root }) {
      ...
      root.innerHTML = `
        <p style="font-size:13px;color:rgba(252, 224, 168,.7);margin:0 0 14px;">Décris ton chantier (photos, surface, délai). ${esc(proNom)} te répond avec un devis chiffré.</p>
        <button id="wdc-open" style="background:#E8940A;color:#14100A;border:none;border-radius:10px;padding:12px 20px;font-weight:700;cursor:pointer;font-family:Poppins,sans-serif;">🔧 Demander un devis</button>
      `;
      root.querySelector('#wdc-open').onclick = () => {
        ...
        ov.innerHTML = `
          <div style="background:#14100A;border:1px solid rgba(232,148,10,.3);border-radius:14px;padding:22px;max-width:520px;width:100%;color:#FCE0A8;font-family:Poppins,sans-serif;">
            <h4 style="font-family:Fraunces,serif;font-style:italic;color:#E8940A;margin:0 0 14px;">Devis chantier — ${esc(proNom)}</h4>
            <div style="display:grid;gap:10px;">
              <input id="wdc-nom" placeholder="Ton nom *" style="width:100%;padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;">
              <input id="wdc-tel" placeholder="WhatsApp *" style="width:100%;padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;">
              <input id="wdc-type" placeholder="Type de travaux (ex: réfection toiture, installation clim)" style="width:100%;padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;">
              <textarea id="wdc-desc" rows="3" placeholder="Décris le chantier — état actuel, ce que tu veux *" style="width:100%;padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;"></textarea>
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                <input id="wdc-surface" type="number" min="0" placeholder="Surface (m²)" style="padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;">
                <input id="wdc-budget" type="number" min="0" placeholder="Budget estimé (FCFA)" style="padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;">
              </div>
              <input id="wdc-delai" placeholder="Délai souhaité (cette semaine, 1 mois…)" style="width:100%;padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;">
              <input id="wdc-adresse" placeholder="Adresse du chantier (quartier, repère)" style="width:100%;padding:10px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;">

              <label style="display:block;font-size:12px;color:rgba(252, 224, 168,.7);margin-top:4px;">Photos du lieu (max ${MAX_PHOTOS}, optionnel)</label>
              <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
                <label style="background:#E8940A;color:#14100A;border:none;border-radius:8px;padding:8px 14px;font-weight:700;cursor:pointer;font-size:12px;display:inline-flex;align-items:center;gap:6px;">
                  <span>📷 Téléverser</span>
                  <input id="wdc-file" type="file" accept="image/*" multiple style="display:none;">
                </label>
                <button type="button" id="wdc-url-toggle" style="background:none;border:1px solid rgba(252, 224, 168,.2);color:#FCE0A8;border-radius:8px;padding:8px 12px;cursor:pointer;font-size:12px;">+ URL</button>
                <span id="wdc-upload-status" style="font-size:11px;color:rgba(252, 224, 168,.55);"></span>
              </div>
              ...
              <div id="wdc-url-row" style="display:none;gap:6px;">
                <input id="wdc-url-input" placeholder="https://i.ibb.co/..." style="flex:1;padding:8px;background:#16201a;border:1px solid rgba(232,148,10,.2);border-radius:8px;color:#FCE0A8;font-family:'Geist Mono',monospace;font-size:12px;">
                <button type="button" id="wdc-url-add" style="background:#E8940A;color:#14100A;border:none;border-radius:8px;padding:0 14px;font-weight:700;cursor:pointer;font-size:12px;">Ajouter</button>
              </div>
            </div>
            <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:16px;">
              <button id="wdc-cancel" style="background:none;border:1px solid rgba(252, 224, 168,.2);color:#FCE0A8;border-radius:8px;padding:10px 16px;cursor:pointer;">Annuler</button>
              <button id="wdc-send" style="background:#E8940A;color:#14100A;border:none;border-radius:8px;padding:10px 20px;font-weight:700;cursor:pointer;">Envoyer la demande</button>
            </div>
          </div>`;
        ...
        ov.querySelector('#wdc-send').onclick = async () => {
          ...
          try {
            ...
            ov.innerHTML = `<div style="background:#14100A;border:1px solid rgba(232,148,10,.3);border-radius:14px;padding:30px;max-width:420px;text-align:center;color:#FCE0A8;font-family:Poppins,sans-serif;"><div style="font-size:42px;margin-bottom:10px;">✓</div><h4 style="font-family:Fraunces,serif;font-style:italic;color:#E8940A;margin:0 0 8px;">Demande envoyée</h4><p style="font-size:14px;color:rgba(252, 224, 168,.75);margin:0 0 18px;">${esc(proNom)} va te répondre avec un devis chiffré.</p><button id="wdc-close" style="background:#E8940A;color:#14100A;border:none;border-radius:8px;padding:10px 22px;font-weight:700;cursor:pointer;">Fermer</button></div>`;
    ...
    async renderDashboard({ proUserId, root }) {
      ...
      root.innerHTML = `
        <div style="margin-bottom:12px;font-size:13px;color:rgba(252, 224, 168,.65);">Demandes de devis reçues. Réponds avec un montant + message ; le client est notifié par WhatsApp.</div>
      ...
      function row(d) {
        ...
        div.innerHTML = `
          ...
          <div style="font-size:12px;color:rgba(252, 224, 168,.7);line-height:1.45;">${esc(d.description)}</div>
          <div style="font-size:11px;color:rgba(252, 224, 168,.55);margin-top:6px;display:flex;gap:10px;flex-wrap:wrap;">
          ...
          <div style="display:flex;gap:6px;margin-top:10px;flex-wrap:wrap;">
            <a href="https://wa.me/${esc(d.client_telephone.replace(/\D/g,''))}?text=${encodeURIComponent('Bonjour ' + d.client_nom + ', je te recontacte au sujet de ta demande sur WOZALI.')}" target="_blank" style="background:none;border:1px solid rgba(232,148,10,.4);color:#FCE0A8;border-radius:6px;padding:5px 10px;text-decoration:none;font-size:12px;">💬 WhatsApp</a>
            <button data-act="devis" style="background:#E8940A;color:#14100A;border:none;border-radius:6px;padding:5px 12px;cursor:pointer;font-size:12px;font-weight:700;">📝 Envoyer un devis</button>
      ...
      if (!list.length) {
        listEl.innerHTML = `<div style="color:rgba(252, 224, 168,.55);font-size:13px;">Aucune demande pour l'instant.</div>`;
        return;
      }
