...
// ════════════════════════════════════════════════════════════════
// POST /api/wozali-pay/bourse-tirage-mensuel (protégé par CRON_SECRET)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const c of candidates) {
      ...
      // Score WOZALI ≥ 80
      if ((Number(c.score_wozali) || 0) < 80) continue;
    ...
    if (gagnantTG) {
      inserts.push({
        ...
        eligible: true,
        score_wozali: gagnantTG.score_wozali,
      });
    }
    if (gagnantBJ) {
      inserts.push({
        ...
        date_tirage: now.toISOString(),
        eligible: true,
        score_wozali: gagnantBJ.score_wozali,
      });
    }
