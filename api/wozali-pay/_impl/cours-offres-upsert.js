...
export default async function handler(req, res) {
  ...
  if (body.id) {
    const { data: ex } = await supabase.from('wozali_cours_offres').select('pro_user_id').eq('id', body.id).maybeSingle();
    if (!ex) return res.status(404).json({ error: 'Introuvable' });
    if (ex.pro_user_id !== userId) return res.status(403).json({ error: 'Pas autorisé' });
    const { data, error } = await supabase.from('wozali_cours_offres').update(payload).eq('id', body.id).select().single();
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json({ offre: data });
  }
  const data = await insertSelf('wozali_cours_offres', 'pro_user_id', userId, payload, res);
  if (!data) return;
  return res.status(200).json({ offre: data });
}
