...
// POST/GET /api/wozali-pay/whatsapp-flush
// Lit les messages pending dans wozali_whatsapp_queue où scheduled_at <= now(),
...
//  3. INBOX        (toujours dispo) → push dans wozali_prestataires.notifications
...
function inferTitle(templateKey, content) {
  ...
  const titles = {
    ...
    A4_concours: "La Bourse de Croissance, c'est ouvert",
    ...
    B4_concours_apprenties: "La Bourse de Croissance, ouvert à toi",
    ...
    PED3_recompenses: 'La Bourse de Croissance, un salaire pour les 10 meilleurs profils de ton pays',
    PED4_avis_clients: 'Les avis clients × 3 contacts',
    PED5_score_wozali: 'Comprends ton Score WOZALI',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: tpls } = await supabase
      .from('wozali_whatsapp_templates')
      .select('key, content')
      .in('key', tplKeys);
    const tplMap = {};
    ...
    for (const m of dueMsgs) {
      ...
      if (!tplContent) {
        await supabase.from('wozali_whatsapp_queue').update({
    ...
    // Log dans wozali_errors_log pour remonter dans l'agrégat monitoring
    try {
      await supabase.from('wozali_errors_log').insert({
        source: 'cron',
        error_msg: String(err.message || err).slice(0, 1000),
        stack: String(err.stack || '').slice(0, 4000),
        url: '/api/wozali-pay/whatsapp-flush',
