...
// POST /api/wozali-pay/vote-share  Body: { user_id, photo_id?, candidature_id? }
// GET  /api/wozali-pay/vote-share?token=xxx  (tracker le clic)
// ================================================================
    ...
    try {
      ...
      if (partage.clicks >= 99 && partage.photo_id) {
        const { data: photo } = await supabase
          ...
          .single();
        if (photo) {
          await supabase.from('badges_wozali').insert({ user_id: photo.user_id, badge_type: 'virale_100' }).select();
        }
      }
