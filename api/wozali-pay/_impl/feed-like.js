  ...
  try {
    ...
    if (photo) {
      ...
      for (const { seuil, type } of seuils) {
        if (totalLikes >= seuil) {
          await supabase.from('badges_wozali').insert({ user_id: photo.user_id, badge_type: type }).select();
        }
      }
      ...
      await supabase.from('badges_wozali').insert({ user_id: photo.user_id, badge_type: niveau }).select();
    }
