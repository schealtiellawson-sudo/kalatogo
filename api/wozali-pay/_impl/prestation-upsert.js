// POST /api/wozali-pay/prestation-upsert
...
export default async function handler(req, res) {
  ...
  if (!body.id) {
    ...
    const data = await insertSelf('wozali_prestations_catalogue', 'pro_user_id', userId, payload, res);
    if (!data) return;
    return res.status(200).json({ prestation: data });
  }
  const data = await updateOwned('wozali_prestations_catalogue', 'pro_user_id', body.id, userId, payload, res);
  if (!data) return;
  return res.status(200).json({ prestation: data });
}
