// POST /api/wozali-pay/portfolio-upsert
...
export default async function handler(req, res) {
  ...
  if (!body.id) {
    ...
    const data = await insertSelf('wozali_portfolio_projets', 'pro_user_id', userId, payload, res);
    if (!data) return;
    return res.status(200).json({ projet: data });
  }
  const data = await updateOwned('wozali_portfolio_projets', 'pro_user_id', body.id, userId, payload, res);
  if (!data) return;
  return res.status(200).json({ projet: data });
}
