...
export async function sendPushToUser(userId, payload = {}) {
  ...
  const { data: subs, error } = await supabase
    .from('wozali_push_subscriptions')
  ...
  const notif = {
    ...
    icon: payload.icon || '/icons/wozali-192.svg',
    badge: payload.badge || '/icons/wozali-192.svg',
    tag: payload.tag || 'wozali-default',
    data: payload.data || {},
  };
  ...
  if (deadIds.length > 0) {
    await supabase.from('wozali_push_subscriptions').delete().in('id', deadIds);
  }
...
/**
 * Handler HTTP : POST /api/wozali-pay/push-send
