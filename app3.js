...
async function afficherTop50(paysFiltre){
  ...
  container.innerHTML=list.map((p,i)=>{
    ...
    return `<div class="top50-card"><div class="top50-rang-col"><span class="top50-rang ${rangCls}">#${rang}</span>${top10}</div><img class="top50-photo" src="${(window._wThumb?window._wThumb(p.photo_profil||p.photo_url||'',160,160):encodeURI(p.photo_profil||p.photo_url||''))}" alt="" decoding="async" onerror="this.onerror=null;this.src='${encodeURI(p.photo_profil||p.photo_url||'')}'"><div class="top50-info"><strong class="top50-nom">${escapeHtml(p.nom_complet||p.nom||'—')}</strong><span>${escapeHtml(p.metier_principal||p.metier||'')}</span><span>${escapeHtml(p.ville||'')} ${p.pays?('· '+escapeHtml(p.pays)):''}</span><div class="top50-score-bar"><div class="top50-score-fill" style="width:${score}%"></div></div></div><a class="top50-voir" href="#" onclick="showPage('profil','${p.user_id||p.id}');return false;">Voir →</a></div>`;
  }).join('');
}
...
(function(){
  ...
  // ── WOZALI Match : bloc déplacé dans wozali-match.js (surcharge, chargé après app3.js). Ancien code retiré 2026-07-26. ──
})();
...
// Contacter un prestataire depuis une carte de résultat (ex: WOZALI Match) →
// ouvre son profil public plutôt qu'un contact direct non défini.
window.contacterAuteur = function(userId) {
  if (!userId) return;
  if (typeof showProfil === 'function') showProfil(userId);
